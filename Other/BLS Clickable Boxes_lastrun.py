...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='C:\\Users\\Jordan\\Documents\\Experiment Building\\6. Backwards Location Span\\BLS Clickable Boxes_lastrun.py',
...
for thisTrial in Trials:
    ...
    # set up handler to look after randomisation of conditions etc
    Responses = data.TrialHandler(nReps=1, method='sequential', 
        extraInfo=expInfo, originPath=-1,
        trialList=data.importConditions('BLS_responses.xlsx', selection=resp_rows),
        seed=None, name='Responses')
    thisExp.addLoop(Responses)  # add the loop to the experiment
    thisResponse = Responses.trialList[0]  # so we can initialise stimuli with some values
    # abbreviate parameter names if possible (e.g. rgb = thisResponse.rgb)
    if thisResponse != None:
        for paramName in thisResponse:
            exec('{} = thisResponse[paramName]'.format(paramName))
    
    for thisResponse in Responses:
        currentLoop = Responses
        # abbreviate parameter names if possible (e.g. rgb = thisResponse.rgb)
        if thisResponse != None:
            for paramName in thisResponse:
                exec('{} = thisResponse[paramName]'.format(paramName))
        ...
        for thisComponent in ResponseComponents:
            if hasattr(thisComponent, "setAutoDraw"):
                thisComponent.setAutoDraw(False)
        # store data for Responses (TrialHandler)
        Responses.addData('mouse.x', mouse.x)
        Responses.addData('mouse.y', mouse.y)
        Responses.addData('mouse.leftButton', mouse.leftButton)
        Responses.addData('mouse.midButton', mouse.midButton)
        Responses.addData('mouse.rightButton', mouse.rightButton)
        Responses.addData('mouse.time', mouse.time)
        Responses.addData('mouse.clicked_name', mouse.clicked_name)
        ...
        if correct == 1:
            ...
            wrongAnswer +=1
            if wrongAnswer == 2:
                resp_locations.finished = True
                         
                         
        # the Routine "Response" was not non-slip safe, so reset the non-slip timer
        routineTimer.reset()
    # completed 1 repeats of 'Responses'
