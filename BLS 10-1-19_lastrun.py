...
resp_cue = visual.TextStim(win=win, name='resp_cue',
    ...
    languageStyle='LTR',
    depth=-29.0);
order_cue = visual.TextStim(win=win, name='order_cue',
    text='Click\nin\nReverse',
    font='Arial',
    pos=(-0.6, 0.40), height=0.05, wrapWidth=None, ori=0, 
    color='white', colorSpace='rgb', opacity=1, 
    languageStyle='LTR',
    depth=-30.0);
...
Intro = data.TrialHandler(nReps=1, method='sequential', 
    extraInfo=expInfo, originPath=-1,
    trialList=data.importConditions('BLS_Instructions.xlsx'),
...
for thisIntro in Intro:
    ...
    Intro.addData('cont_text.started', cont_text.tStartRefresh)
    Intro.addData('cont_text.stopped', cont_text.tStopRefresh)
    # the Routine "Instructions" was not non-slip safe, so reset the non-slip timer
    routineTimer.reset()
# completed 1 repeats of 'Intro'


# set up handler to look after randomisation of conditions etc
Trials = data.TrialHandler(nReps=1, method='sequential', 
    extraInfo=expInfo, originPath=-1,
    trialList=data.importConditions('BLS_Trials.xlsx'),
...
for thisTrial in Trials:
    ...
    Stimuli = data.TrialHandler(nReps=1, method='sequential', 
        extraInfo=expInfo, originPath=-1,
        trialList=data.importConditions('BLS_Stimuli.xlsx', selection=rows),
    ...
    for square in squares:
        ...
        square.color = 'white'
    # keep track of which components have finished
    ResponseComponents = [grid, key_resp, polygon, polygon_2, polygon_3, polygon_4, polygon_5, polygon_6, polygon_7, polygon_8, polygon_9, polygon_10, polygon_11, polygon_12, polygon_13, polygon_14, polygon_15, polygon_16, polygon_17, polygon_18, polygon_19, polygon_20, polygon_21, polygon_22, polygon_23, polygon_24, polygon_25, mouse, resp_cue, order_cue]
    ...
    while continueRoutine:
        ...
        # *resp_cue* updates
        if resp_cue.status == NOT_STARTED and tThisFlip >= 0-frameTolerance:
            # keep track of start time/frame for later
            resp_cue.frameNStart = frameN  # exact frame index
            resp_cue.tStart = t  # local t and not account for scr refresh
            resp_cue.tStartRefresh = tThisFlipGlobal  # on global time
            win.timeOnFlip(resp_cue, 'tStartRefresh')  # time at next scr refresh
            resp_cue.setAutoDraw(True)
        
        # *order_cue* updates
        if order_cue.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
            # keep track of start time/frame for later
            order_cue.frameNStart = frameN  # exact frame index
            order_cue.tStart = t  # local t and not account for scr refresh
            order_cue.tStartRefresh = tThisFlipGlobal  # on global time
            win.timeOnFlip(order_cue, 'tStartRefresh')  # time at next scr refresh
    ...
    while continueRoutine:
        ...
        # *rdy_txt* updates
        if rdy_txt.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
            # keep track of start time/frame for later
            rdy_txt.frameNStart = frameN  # exact frame index
            rdy_txt.tStart = t  # local t and not account for scr refresh
            rdy_txt.tStartRefresh = tThisFlipGlobal  # on global time
            win.timeOnFlip(rdy_txt, 'tStartRefresh')  # time at next scr refresh
            rdy_txt.setAutoDraw(True)
        
        # *rdy_key_resp* updates
        waitOnFlip = False
        if rdy_key_resp.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
            # keep track of start time/frame for later
            rdy_key_resp.frameNStart = frameN  # exact frame index
            rdy_key_resp.tStart = t  # local t and not account for scr refresh
            rdy_key_resp.tStartRefresh = tThisFlipGlobal  # on global time
